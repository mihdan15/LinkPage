...
const SOCIAL_ICONS: Record<SocialLink['platform'], { icon: React.ReactNode; color: string }> = {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`w-32 h-32 rounded-full flex items-center justify-center border-4 shadow-lg bg-gray-200 dark:bg-gray-700 ${profile.avatarUrl ? 'hidden' : ''}`}
          style={{ borderColor: primaryColor }}
        >
          ...
          {enabledSocialLinks.map((link, index) => {
            ...
            return (
              <motion.a
                ...
                className="p-2 rounded-full bg-white/80 dark:bg-gray-800/80 shadow-md hover:shadow-lg transition-all cursor-pointer"
